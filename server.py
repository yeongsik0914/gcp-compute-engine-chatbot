...
def chat():
    api_key = get_api_key()
    if not api_key:
        return jsonify({"error": "GEMINI_API_KEY 환경변수가 설정되지 않았습니다. 서버 환경변수를 확인해 주세요."}), 500
    ...
    def generate_stream():
        try:
            ...
            # Tools for Google Search as provided by user
...
if __name__ == "__main__":
    ...
    print(f"[*] Gemini Chatbot 서버 시작: http://{host}:{port}")
